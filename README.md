...
В каждом датацентре четное количество машин с NGINX. Они расположены с использованием VRRP, чтобы при выключении балансировщика включался запасной. На них будет производиться балансировка с помощью протокола динамической маршрутизации iBGP.

Далее NGINX проксирует запросы на сервера.
