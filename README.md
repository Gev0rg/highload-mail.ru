...
--------|---------|-----------
MailBox | Денормолизуем бд, убрав таблицу таблицу User и перенеся все поля в таблицу MailBox, также перенесем сюда же содержимое таблицы Folders в виде [{ folderId, name }] | postgresql
...
FilesMeta | Необходимо хранить мета-данные файла, такие как имя, url в s3, id письма для проверки доступа к файлу, а также массив пользователей, имеющих доступ к файлу | postgresql
Files   | Файлы хранить в kv blob-storage (s3) | Minio
Sessions | Сессии пользователей | Reddis
...
| Minio       | Хранение вложенных файлов      | Довольно хорошее решение, обладающие сравнительно неплохими показателями производительности как на чтение, так и на запись, а также обеспечивает высокую масштабируемость, opensource |
