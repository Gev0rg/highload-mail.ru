...
| Решение                 | Обоснование |
| ----------------------- | ----------- |
| Шардированый PostgreSQL | Для распараллеливания запросов при больших нагрузках |
| Elasticsearch           | Для быстрого поиска и снижения нагрузки на основную базу данных с письмами |
| s3mail.ru               | Для быстро доступа к файлам |
...
- Схема резирвирования - 2N - это две полные параллельные системы для каждого элемента N. То есть каждый элемент N в такой схеме дублируется, а нагрузка одинакова на двух элементах. При такой системе резервирования сбой одного или нескольких элементов N или выход из строя одной из систем не повлияют на работу всей системы в целом.
- Отказоустойчивость БД
  - Используемые БД устойчивы к разделению
  - Обеспечение надежности PostgreSQL с помощью асинхронной master-slave репликации с использованием Patroni и etcd 
...
| s3mail.ru      | CyberStore 472S 12GB/s Storage Server / Intel Xeon Silver 4316 Processor 20 Cores /  RAM 16x128GB / HDD 72x2.4TB |  20   | 5 926 | €  1 778 000  |
