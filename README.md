...
Изучив плотность населения по регионам в РФ[^8], прохождение магистральных кабелей[^9], население федеральных округов[^10], а так же особенности работы почты, можно выбрать одну локацию для размещения единственного дата центра - Москва, а также следующие локации для размещения cdn:
* Москва
* Санкт-Петербург
* Екатеринбург
* Владивосток
...
Рассчитаем процент пользователей для почты Mail.ru от всех жителей России. В России проникновение интернета составляет 88.2%[^11]. Также процент проникновения Алисы равен 51 / (146 * 0.88) * 100% = 39.7%, тогда коэффициент для пользователей почты равен 88.2% * 0.397 = 35%

### Балансировка

Так как мы имеем только один ЦОД и несколько CDN, то необходимо распределить нагрузку только между ними.

GeoDNS сервера определяют локацию пользователя и возвращают IP адрес, за которым находится CDN. CDN, в свою очередь, знают об адресе ЦОДа.

### Итоговое покрытие

В итоге получилось 1 местоположение ЦОДа и 4 местоположения для CDN наших.
